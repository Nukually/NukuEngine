...
NkPipeline::~NkPipeline() {
  vkDestroyShaderModule(NkDevice.device(), vertShaderModule, nullptr);
  vkDestroyShaderModule(NkDevice.device(), fragShaderModule, nullptr);
  vkDestroyPipeline(NkDevice.device(), graphicsPipeline, nullptr);
}

std::vector<char> NkPipeline::readFile(const std::string& filepath) {
...
void NkPipeline::createShaderModule(const std::vector<char>& code, VkShaderModule* shaderModule) {
  ...
  if (vkCreateShaderModule(NkDevice.device(), &createInfo, nullptr, shaderModule) != VK_SUCCESS) {
