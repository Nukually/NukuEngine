...
namespace std {
template <>
struct hash<nk::NkModel::Vertex> {
  size_t operator()(nk::NkModel::Vertex const &vertex) const {
    size_t seed = 0;
...
NkModel::NkModel(VulkanContext &device, const NkModel::Builder &builder) : NkDevice{device} {
...
void NkModel::createVertexBuffers(const std::vector<Vertex> &vertices) {
  ...
  NkDevice.copyBuffer(stagingBuffer.getBuffer(), vertexBuffer->getBuffer(), bufferSize);
}
...
void NkModel::createIndexBuffers(const std::vector<uint32_t> &indices) {
  ...
  NkDevice.copyBuffer(stagingBuffer.getBuffer(), indexBuffer->getBuffer(), bufferSize);
}
